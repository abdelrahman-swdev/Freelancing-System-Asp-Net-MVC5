...
namespace FreelancingSystemProject.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model, HttpPostedFileBase upload)
        {
            if (ModelState.IsValid)
            {
                ViewBag.UserType = new SelectList(db.Roles.Where(r => r.Name != MagicStrings.AdminRole).ToList());
                
                string path = Path.Combine(Server.MapPath("~/uploads"), upload.FileName);
                upload.SaveAs(path);

                var user = new ApplicationUser { FirstName = model.FirstName, LastName = model.LastName,imageUrl = upload.FileName, UserName = model.Email, Email = model.Email, PhoneNumber = model.PhoneNumber, UserType = model.UserType };
        ...
        public ActionResult EditProfile(EditProfileViewModel model, HttpPostedFileBase upload)
        {
            ...
            else
            {
                ...
                CurrentUser.imageUrl = upload.FileName;
                CurrentUser.PhoneNumber = model.PhoneNumber;
                CurrentUser.PasswordHash = newPasswordHashed;

                db.Entry(CurrentUser).State = System.Data.Entity.EntityState.Modified;
                db.SaveChanges();
                ViewBag.Message = "Your Account Updated Successfully";
            }
            return View(model);
        }
